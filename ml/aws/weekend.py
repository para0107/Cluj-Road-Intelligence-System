...
def plan(queue: list[str], deadline: datetime, gpu_factor: float,
    ...
    for exp_id in queue:
        ...
        for seed in spec.seeds:
            # freeze_epochs matters: a frozen-backbone epoch costs 357 s against 547 s
            # for a full one, so ignoring it over-estimates a 10+10 run by ~20%.
...
# Running
# ---------------------------------------------------------------------------
def resolve_data_yaml(spec, data_yaml: str, data_root: Optional[str]) -> str:
    """
    The dataset yaml THIS spec must train on.

    E9 and E10 do not vary the model at all - they vary how `stage_dataset.py` built
    the splits. Passing one `--data` to the whole queue would train every one of them
    on the standard split and produce numbers that look entirely plausible while
    measuring nothing. That is the most dangerous failure mode in this programme,
    because nothing downstream can detect it.

    With `--data-root DIR`, a spec whose `data_variant` is not "standard" resolves to
    `DIR/staged_<variant>/dataset_nrdd2024_research.yaml`, matching the layout the
    prep step in WEEKEND2_PLAN.md section 4 produces. "standard" always uses `--data`.

    Raises:
        FileNotFoundError: the variant's staged yaml is missing. Fail here, loudly,
            rather than silently falling back to the wrong dataset.
    """
    variant = getattr(spec, "data_variant", "standard")
    if variant == "standard":
        return data_yaml
    if not data_root:
        raise FileNotFoundError(
            f"{spec.id} needs data_variant '{variant}', which is a different staged "
            f"split from --data. Pass --data-root <dir> containing "
            f"staged_{variant}/, or drop this experiment from the queue. Refusing to "
            f"train it on the standard split, which would silently measure nothing."
        )
    candidate = Path(data_root) / f"staged_{variant}" / "dataset_nrdd2024_research.yaml"
    if not candidate.exists():
        matches = sorted((Path(data_root) / f"staged_{variant}").glob("*.yaml"))
        if not matches:
            raise FileNotFoundError(
                f"{spec.id}: no dataset yaml under {candidate.parent}. Stage it first:\n"
                f"    {spec.stage_command('$SRC', str(candidate.parent))}"
            )
...
def run_queue(
    ...
    epochs: Optional[int] = None, data_root: Optional[str] = None,
) -> int:
    cutoff = deadline - timedelta(hours=EXPORT_RESERVE_H)
    ...
    for exp_id in queue:
        ...
        for seed in spec.seeds:
            ...
            h = estimate_hours(n_ep, spec.imgsz, 1, gpu_factor, spec.freeze_epochs)
            ...
            try:
                spec_data = resolve_data_yaml(spec, data_yaml, data_root)
            ...
            entry = {
                ...
                "data_variant": variant, "data_yaml": spec_data,
...
def main() -> int:
    ...
    ap.add_argument("--data", help="dataset yaml for standard-variant experiments")
    ap.add_argument("--data-root", help="directory holding staged_<variant>/ trees for "
                    "E9/E10, whose splits differ from the standard one")
