...
Reads severity_results.json (the output of validate_severity.py) and runs
the Enricher on all frames. Frames without GPS coordinates are forwarded
with enrichment fields set to None — this is expected behaviour when
no GPS telemetry was available for the survey session.

This script chains directly from severity_results.json because that is the
last validated JSON in the pipeline at this stage. In the full operational
pipeline the Enricher receives output from the SeverityClassifier, which
is equivalent.

API compliance:
  Nominatim — max 1 request/second enforced (delay_nominatim_s=1.1).
  The User-Agent is set to a project-specific string in pipeline/enricher.py.
  OSM data is used under the ODbL licence.
  Open-Meteo is used under its non-commercial free tier.

Outputs
-------
  data/validation_nrdd_2024/enriched/
      enriched.json           -- all frames with enrichment fields added
      enrichment_report.json  -- statistics: GPS rate, API success/fail counts
...
# ---------------------------------------------------------------------------
# Logging — must be set up before importing pipeline modules that use
...
def main(
    ...
    # ------------------------------------------------------------------
    # Load severity_results.json — real data, no hardcoded values
    # ------------------------------------------------------------------
    if not SEVERITY_JSON.exists():
        logger.error("severity_results.json not found: %s", SEVERITY_JSON)
        logger.error(
            "Run scripts/validate_severity.py first to generate it."
        )
        raise SystemExit(1)

    logger.info("Loading severity results: %s", SEVERITY_JSON)
    with SEVERITY_JSON.open("r", encoding="utf-8") as f:
        severity_data = json.load(f)

    frames: List[dict] = severity_data.get("frames", [])
    logger.info(
    ...
    # ------------------------------------------------------------------
    # Warn about GPS situation
    # ------------------------------------------------------------------
    n_with_gps = sum(
        1 for fr in frames
        if fr.get("latitude") is not None and fr.get("longitude") is not None
    )
    n_without_gps = len(frames) - n_with_gps

    logger.info(
        "GPS summary: %d frames with GPS coordinates, %d without",
        n_with_gps, n_without_gps,
    )
    if n_with_gps == 0:
        logger.warning(
            "No GPS coordinates found in severity_results.json. "
            "All enrichment API calls will be skipped. "
            "Enrichment fields will be null for all detections. "
            "To get meaningful enrichment results, use a GPS-synchronised "
            "survey session (.gpx file paired with dashcam footage)."
        )
    ...
    results  = enricher.run(frames, output_dir=str(OUT_DIR))
    ...
    # Per-class enrichment rate (only meaningful if GPS was available)
    ...
    report = {
        ...
        "n_frames_without_gps": n_without_gps,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Skip OSM Overpass road importance / infra proximity lookups",
    )
    parser.add_argument(
